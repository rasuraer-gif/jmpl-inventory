...
const DeflashingModule = (() => {
  ...
  function rejectBatch() {
    ...
    const session = Auth.getSession();
    const batch = DB.Batches.find(batchId) || {};
    DB.RejectionTracker.insert({ batchId, batchNo: batch.batchNo||'', jmrefNo: batch.jmrefNo||'', partNo: batch.partNo||'', stage:'deflashing', qty:getInputQty(batchId), date:new Date().toISOString(), reason, rejectedBy:session?.userId });
