...
const WaitingVisualModule = (() => {
      ...
      <div class="card animate-in">
        ...
        <div class="table-wrap">
          <table class="data-table">
            <thead>
              <tr><th>Batch</th><th>Part No</th><th>JMREF</th><th>WIP Qty</th><th>Rack Location</th><th>Actions</th></tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
          ...
          <div id="wv-allocate-stock-fields" class="hidden">
            ...
            <div class="form-group">
              ...
              <div class="flex gap-3 mt-2">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;"><input type="radio" name="wv-allocate-type" id="wv-allocate-type-inhouse" value="inhouse" checked onchange="WaitingVisualModule.updateAllocateDynamicBatchNo()"> <span>In-House (D)</span></label>
  ...
  function updateAllocateDynamicBatchNo() {
    ...
    const typeCode = type === 'subcontractor' ? 'S' : 'D';
    const subBatchInput = document.getElementById('wv-allocate-sub-batch-no');
    if (subBatchInput) {
      if (trNo && dayStr) {
        subBatchInput.value = `${_activeBatch.jmrefNo}-${trNo}-${dayStr}-${typeCode}`;
      } else {
        subBatchInput.value = '';
      }
    }
          ...
          <div id="wv-stock-fields" class="hidden">
            ...
            <div class="form-row-2">
              <div class="form-group" style="flex:1;">
                ...
                <input type="date" id="wv-date-move" class="form-control" value="${new Date().toISOString().slice(0,10)}" onchange="WaitingVisualModule.updateDynamicBatchNo()">
              </div>
              <div class="form-group" style="flex:1;">
